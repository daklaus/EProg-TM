 ...
 * Member functions
 */
Bruch::Bruch(long z, unsigned long n) {
	setZaehler(z);
	setNenner(n);
}
...
void Bruch::setNenner(unsigned long n) {
	if (n <= 0)
		throw range_error("Denominator must not be less or equal zero");
	nenner = n;
}
...
Bruch Bruch::operator +(const Bruch& rhs) const {
	Bruch ret(*this);
	ret.zaehler = ret.zaehler * rhs.nenner + rhs.zaehler * ret.nenner;
	ret.nenner *= rhs.nenner;
	return ret;
}

Bruch Bruch::operator -(const Bruch& rhs) const {
	Bruch ret(*this);
	ret.zaehler = ret.zaehler * (long) rhs.nenner - rhs.zaehler
			* (long) ret.nenner;
	ret.nenner *= rhs.nenner;
	return ret;
}
...
Bruch Bruch::operator /(const Bruch& rhs) const {
	...
	if (rhs.zaehler < 0)
		ret.zaehler *= -1;
	ret.zaehler *= (long) rhs.nenner;
	ret.nenner *= labs(rhs.zaehler);
	return ret;
}
